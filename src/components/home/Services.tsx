	...
	return (
		<div className="relative isolate overflow-hidden bg-gradient-to-b from-blue-50 via-white to-blue-50 py-24 sm:py-32">
			<div className="absolute inset-x-0 top-0 -z-10 transform-gpu overflow-hidden blur-3xl">
				<div className="relative left-1/2 aspect-[1155/678] w-[36.125rem] -translate-x-1/2 bg-gradient-to-tr from-[#1d4ed8] to-[#93c5fd] opacity-30 animate-gradient-slow"></div>
			</div>
				...
				<div className="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
					<dl className="grid max-w-xl grid-cols-1 gap-x-8 gap-y-16 lg:max-w-none lg:grid-cols-3">
						{services.map((service) => (
							<div key={service.name} className="relative group cursor-pointer">
								<div className="absolute -inset-4 rounded-2xl bg-blue-50 opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:scale-[1.02]"></div>
								<div className="relative flex flex-col transition-all duration-300">
									<dt className="flex items-center gap-x-3">
										<div className="rounded-lg bg-blue-600 p-2 ring-1 ring-blue-600/10 transition-transform duration-300 group-hover:scale-110 group-hover:shadow-lg">
											<service.icon className="h-5 w-5 flex-none text-white transition-transform duration-300 group-hover:scale-110" aria-hidden="true" />
										</div>
										<span className="text-lg font-semibold leading-7 text-gray-900 transition-colors duration-300 group-hover:text-blue-600">
											{service.name}
										</span>
									</dt>
									<dd className="mt-4 flex flex-auto flex-col text-base leading-7 text-gray-600">
										<p className="flex-auto transition-colors duration-300 group-hover:text-gray-900">
											{service.description}
										</p>
									</dd>
								</div>
